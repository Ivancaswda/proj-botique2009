...
const Cart = () => {
  ...
  //  localStorage.setItem('newAddress', JSON.stringify(e.target.value))
    ...
    const handlePlaceOrder = () => {
        ...
        if (delOption === null) {
            toast.error('Выберите способ доставки перед тем как оплачивать!');

            const deliElement = document.getElementById('deli');
            if (deliElement) {
                deliElement.scrollIntoView({ behavior: 'smooth' });
                setVibrate(true)
                setTimeout(() => {
                setVibrate(false)
                }, 1500)
            ...
            {cartData.length > 0 ? (
                                    ...
                                    <div className='w-100'>
                                        <div className='flex justify-between items-center '>
                                            ...
                                            <div className='flex items-center gap-6'>
                                                <button onClick={() => {
                                                    updateQuantity(item._id, item.retOption, item.quantity - 1)
                                        ...
                                        <div className='flex flex-col items-start justify-start gap-5 mt-2 '>
                                            ...
                                            <div className='w-full flex items-center justify-start gap-5'>
                                                ...
                                                <svg className='cursor-pointer' onClick={() => {
                                                    updateQuantity(item._id, item.retOption, 0) // to remove this product we provide zero
                ...
                <div className='flex justify-end bg-gray-100 p-6 rounded-2xl '>
                    ...
                    <div className=' p-4 ' style={{width: '70%'}}>
                        ...
                        { globalUser ?
                            (<div className='bg-gray-100 p-4  ' style={{width: '80%'}}>

                                <div className='flex mb-4 '>
                                    <h1 className='text-2xl text-left font-semibold'>Способ Доставки</h1>
                                </div>
                                <div id='deli' className='flex  gap-2  background'>
                                    <div onClick={() => {
                                        setDelOption('Курьер')
                                        toast("Вы выбрали доставку курьером!", {
                                         ...
                                         className={`${vibrate ? 'animat' : ''}  p-3 rounded-2xl text-gray-100 font-semibold  bg-blue-600 flex items-center cursor-pointer gap-2 ${delOption === 'Курьер' ? 'bg-blue-400' : ''}`}>
                                        <h1>Курьером</h1>
                                    ...
                                    <div onClick={() => {
                                        setDelOption('Quant-express')
                                        toast("Вы выбрали доставку через  Quant-express!", {
                                            position: "top-center", // Позиция: по центру сверху
                                            autoClose: 2000, // Уведомление исчезает через 2 секунды
                                            style: { backgroundColor: "black", color: 'white' }, // Оранжевый цвет фона
                                        });
                                    }}
                                        className={`${vibrate ? 'animat' : ''}  p-3 rounded-2xl text-sm text-gray-100 font-semibold  bg-blue-600 flex items-center cursor-pointer gap-2 ${delOption === 'Quant-express' ? 'bg-blue-400' : ''}`}>
                                        <h1>Quant-express</h1>
                                        <svg width='20px' height='20px' xmlns="http://www.w3.org/2000/svg"
                                             viewBox="0 0 640 512">
                                            <path fill='white'
                                    ...
                                    <div onClick={() => {
                                        setDelOption('Пункт выдачи')
                                        toast("Вы заберете заказ в пункте выдачи!", {
                                            position: "top-center", // Позиция: по центру сверху
                                            autoClose: 2000, // Уведомление исчезает через 2 секунды
                                            style: { backgroundColor: "black", color: 'white' }, // Оранжевый цвет фона
                                        });
                                    }}
                                        className={`${vibrate ? 'animat' : ''}  p-3 rounded-2xl text-sm text-gray-100 font-semibold  bg-blue-600 flex items-center cursor-pointer gap-2 ${delOption === 'Пункт выдачи' ? 'bg-blue-400' : ''}`}>
                                        <h1>В пункте выдачи</h1>
                                        <svg width='20px' height='20px' xmlns="http://www.w3.org/2000/svg"
                                             viewBox="0 0 640 512">
                                            <path fill='white'
                                 ...
                                 <div  className='flex items-center  mb-8'>
                                     <div className='flex items-center gap-2'>
                                         <h1 className='text-lg font-semibold'>Что бы выбрать способ доставки у вас должен быть <span className='text-blue-700'>аккаунт</span></h1>
                                         <button onClick={() => {
                                             navigate('/login')
                                ...
                                <div
                                    ...
                                    <div style={{display: "flex", alignItems: "center", gap: "10px"}}>
                                        ...
                                        {!isEmailEditing ? (
                                            ...
                                            >
              {cleanedEmail ? cleanedEmail : 'Вы пока не создали свой аккаунт!'}
        </span>
                                        ) : (
                                            <input value={editableEmail} onChange={(e) => {
                                                setEditableEmail(e.target.value)
                                                localStorage.setItem('newEmail', JSON.stringify(e.target.value))
                                            }}
                                ...
                                <h1>Домашний Адрес:</h1>
                                    ...
                                    <div style={{display: "flex", alignItems: "center", gap: "10px"}}>
                                        ...
                                        {!isAdressEditing ? (
                                            ...
                                            >
          {adress ? adress : 'Вы пока  не установили адрес!'}
        </span>
                                        ) : (
                                            <InputMask mask="***********, **********, **********, 999"
                                                       maskChar=" "
                                                ...
                                                {(inputProps) => <input
                                                                            ...
                                                                            className="input-field" {...inputProps} />}
                                        ...
                                        <button
                                            onClick={() => {
                                                if (isAdressEditing) {
                                                    ...
                                                    toast("Адрес доставки был успешно изменен!", {
                                                        autoClose: 2000, // Уведомление исчезает через 2 секунды
                            ...
                            <div
                                ...
                                <div style={{display: "flex", alignItems: "center", gap: "10px"}}>
                                    ...
                                    {!isEditing ? (
                                        ...
                                        >
          {phoneNumber ? phoneNumber : 'Вы пока  не установили номер!'}
        </span>
                                    ) : (
                                        <InputMask mask="7-(999)-999-99-99"
                                                   ...
                                                   value={phoneNumber} onChange={(e) => {
                                                        setPhoneNumber(e.target.value)
                                                            localStorage.setItem('newPhone', JSON.parse(e.target.value))
                                        }}>
